//
//  SomeoneBalanceViewController.m
//  kibatina
//
//  Created by beyond on 2020/03/26.
...
@interface SomeoneBalanceViewController ()<UITableViewDataSource, UITableViewDelegate>
{
...
@implementation SomeoneBalanceViewController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = [NSString stringWithFormat:@"%@的收支",_familyDict[@"relation"]];
    // 返回按钮颜色
...
- (void)loadData
{
    ...
    if(_familyDict){
        ...
        for (NSDictionary *item in _balanceArr) {
            if([item[@"family"] isEqualToString:_familyDict[@"relation"]]){
                [resultArr addObject:item];
            }
        }
        _balanceArr = [NSMutableArray arrayWithArray:resultArr];
    }
    if(_tableView){
